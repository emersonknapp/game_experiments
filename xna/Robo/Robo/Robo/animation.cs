...
namespace Robo
{
   class Animation
   {
      ...
      public void Draw(SpriteBatch spriteBatch)
      {
         if (Active)
         {
            int width = Util.round(FrameWidth * scale);
            int height = Util.round(FrameHeight * scale);
            destinationRect = new Rectangle(Util.round(Position.X), Util.round(Position.Y), width, height);
